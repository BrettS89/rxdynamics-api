...
exports.createAccount = async data => {
  ...
  if (existingEmployee) 
    throw { message: 'Email already exists for an employee', status: 400 };
...
exports.login = async data => {
  ...
  if (!employee) throw { message: 'no employee with this email exists', status: 404 };
...
exports.changePasswordByAdmin = async data => {
  if (!data.password || !data.id) 
    throw { message: 'please provide an id and password', status: 400 };

  let employee = await Employee.findById(data.id);
  if (!employee) throw { message: 'could not find employee with this id', status: 404 };
